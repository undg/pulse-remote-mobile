...
let sharedState: ConfigState = { config: { ...DEFAULTS, serverUrl: initialServerUrl }, serverUrl: initialServerUrl, hasUrl: Boolean(DEFAULTS.hostname), loading: true }
...
export function useConfig() {
	useEffect(() => {
		...
		if (!loadingPromise) {
			loadingPromise = loadConfig()
				.then(value => {
					setSharedState({ config: value, serverUrl: value.serverUrl, hasUrl: Boolean(value.hostname), loading: false })
				})
				.finally(() => {
					loadingPromise = null
				})
	...
	const { config, serverUrl, hasUrl, loading } = useSyncExternalStore(subscribe, getSnapshot, getSnapshot)
	...
	const updateConfig = useCallback((partial: Partial<Omit<Config, 'serverUrl'>>) => {
		...
		const next: Config = { hostname, port, endpoint, minVolume, maxVolume, stepVolume, showMonitoredSources, serverUrl: nextServerUrl }
		setSharedState({ config: next, serverUrl: nextServerUrl, hasUrl: Boolean(hostname), loading: false })
		saveConfig(next)
	}, [])
	...
	const resetConfig = useCallback(() => {
		...
		setSharedState({ config: next, serverUrl: nextServerUrl, hasUrl: Boolean(next.hostname), loading: false })
		saveConfig(next)
	}, [])
