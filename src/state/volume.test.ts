...
const baseStatus: PrapiStatus = {
  ...
  sinkInputs: [
    { id: 10, label: "Input 1", sinkId: 1, volume: 30, muted: false },
  ],
  sources: [
    {
      id: 20,
...
describe("useVolumeStore", () => {
  ...
  it("optimistically updates sink volume and unblocks later", () => {
    ...
    expect(result.current.sinks.find((s) => s.name === "sink-1")?.volume).toBe(
      60,
    );
    expect(mockSend).toHaveBeenCalledWith({
      action: Action.SetSinkVolume,
      payload: { name: "sink-1", volume: 60 },
    });
