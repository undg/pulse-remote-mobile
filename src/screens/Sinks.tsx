...
export function SinksScreen() {
  ...
  const {
    ...
    wsStatus,
    reconnect,
  } = useVolumeStore(serverUrl);

  const data = useMemo(() => sinks ?? [], [sinks]);
  const sinkById = useMemo(
    () => Object.fromEntries((data ?? []).map((s) => [s.id, s])),
    [data],
  );
    ...
    () =>
      PanResponder.create({
        ...
        onPanResponderMove: (_, gestureState) => {
          ...
          const hovered = Object.entries(sinkLayouts.current).find(
            ([, rect]) => {
          ...
          const scrollStep = 20;
          const containerH =
            containerSizeRef.current.height || containerSize.height || 0;
          if (containerH > 0) {
            if (viewportY < edgeThreshold) {
              const nextOffset = Math.max(
                0,
                scrollOffsetRef.current - scrollStep,
              );
              ...
              const maxOffset = Math.max(
                0,
                contentHeightRef.current - containerH,
              );
    ...
    ({
      ...
      return (
        <View
          style={[
            styles.card,
            {
              ...
              borderColor:
                hoverSinkId === item.id ? colors.primary : colors.surfaceMuted,
            },
          ]}
          ref={(ref) => {
        ...
        >
          ...
          >
            {children.length === 0 ? (
              <Text style={[styles.emptyChild, { color: colors.muted }]}>
                No inputs attached
              </Text>
            ) : (
      ...
      {draggingId !== null ? (
        <View
          style={[
            styles.dragOverlay,
            {
              top: dragPos.y - containerOffset.y - 28,
              left: dragPos.x - containerOffset.x - 140,
              backgroundColor: colors.surface,
        ...
        >
          <Text
            style={[styles.dragLabel, { color: colors.text }]}
            numberOfLines={1}
          >
            {dragLabel}
          </Text>
