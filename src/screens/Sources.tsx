...
export function SourcesScreen() {
  ...
  const { sources, setSourceVolume, toggleSourceMuted, wsStatus, reconnect } =
    useVolumeStore(serverUrl);

  const data = useMemo(() => {
    const filtered = (sources ?? []).filter((src) =>
      config.showMonitoredSources ? true : !src.monitored,
    );
    return filtered;
    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <View style={[styles.card, { backgroundColor: colors.surface }]}>
          ...
          />
          {item.monitored && !config.showMonitoredSources ? (
            <Text style={[styles.mutedFlag, { color: colors.muted }]}>
              Hidden monitored source
            </Text>
          ) : null}
        </View>
      )}
