...
describe('useWebSocketClient', () => {
	...
	it('connects, parses JSON, and sends payloads', () => {
		...
		const factory = jest.fn(() => socket as unknown as WebSocket) as unknown as typeof WebSocket
		;(factory as any).OPEN = MockWebSocket.OPEN
		global.WebSocket = factory

		const { result } = renderHook(() => useWebSocketClient({ url: 'ws://test' }))
	...
	it('reconnects after close using backoff', () => {
		...
		renderHook(() => useWebSocketClient({ url: 'ws://reconnect', reconnectIntervalsMs: [50, 100], reconnect: true }))
